...
function isValidClaimDate(metaDataDate, claimDate1) {
    const claimDate = new Date(claimDate1);
    const thirtyDaysBefore = new Date(claimDate1);
  ...
  try {
    ...
    if (!isValidClaimDate(formattedDateStr, claimDateStr)) {
      return res
        .status(200)
        .json({
          message: "Please upload images that are taken recently",
          claimDate: claimDate,
          imageDate: "Date not found."
        });
    } else {
      return res.status(200).json({ message: "Valid Image", claimDate: claimDate, imageDate: formattedDate });
    }
  } catch (error) {
    console.log("claimDate: ", claimDate);
    console.error("Error reading EXIF data:", error.message);
    return res.status(200).send({ message: "Please check your image. The image doesn't contain any metadata.", claimDate: claimDate, imageDate: "Invalid Image" });
  }
});
  ...
  try {
    ...
    if (jsonContentMatch) {
      ...
      try {
        ...
        console.log(("matnum: ", matnum));
        const percentNum = matnum.replace(" ", "");
        const matchingPercentage = parseInt(percentNum.replace("%", "")); // Convert to number for comparison
        ...
        const result = {
          ...
          "Matching percentage": matchingPercentage, // Trim to remove any extra spaces
